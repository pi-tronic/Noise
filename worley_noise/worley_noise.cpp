...
#define SECTIONS 1000         // SECTIONS x SECTIONS field
...
int noice_generator(int* nodeArray, int x, int y, int width, int height) {
    ...
    // lets out sections that are not existing (world border)
    for (int a = 0; a < 3; a++) {
	...
	return (1- (min_dist / (double)get_dist(0, 0, width/(SECTIONS*AMOUNT), height/(SECTIONS*AMOUNT)))) * 255;
}
...
/*
    ...
    for (int i = 0; i < (SECTIONS*SECTIONS * AMOUNT); i++) {
        draw_point(surface, pixelArray, nodeArray[i], nodeArray[i+1], 5, width, height, 255, 255, 0);
